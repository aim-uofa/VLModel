...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            sys_func = lambda msg: f"<|start_header_id|>system<|end_header_id|>\n\n{msg}<|eot_id|>"
            ret = sys_func(self.system)
            for i, (role, message) in enumerate(messages):
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    ret += "<|start_header_id|>" + role + "<|end_header_id|>\n\n" + message + "<|eot_id|>"
                    if i == len(messages) - 1 and role == role[0]:  # Assistant end
    ...
    def prepare_images(self):
        ...
        for i in range(0, len(self.messages), 2):
            if isinstance(self.messages[i][1], tuple):
                image = self.messages[i][1][1]
                images.append(image)
        if len(images) == 0:
            return None
        return images
...
# Conversation Template for LLaMA3-instruct
